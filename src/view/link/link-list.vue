...
<script>
  ...
  import {deleteLink, listLinkPage, restoreLink, search} from "../../api/link";
    ...
    data() {
      return {
        columns: [
          ...
          {title: "链接名", key: "name", align: "left"},
          {title: "链接类型", key: "type", align: "left", tooltip: true},
          {
            title: "创建时间",
            key: "createTime",
            align: "left",
    ...
    methods: {
      changePage(current) {
        ...
        if (this.searchFlag) {
          ...
          this.listLinkPage(current, this.pageSize);
        }
      },
      edit(row) {
        this.$router.push({
      ...
      listLinkPage(page, count) {
        this.loading = true;
        listLinkPage(page, count).then(res => {
          this.tableData = res.data.data;
          this.loading = false;
          this.current = page;
          this.totalCount = parseInt(res.data.fields.totalCount);
