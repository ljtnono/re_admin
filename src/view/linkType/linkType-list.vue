  ...
  <div>
    <Card>
      <Form ref="searchForm" :model="searchForm" inline>
        <FormItem prop="name">
          <Input type="text" v-model="searchForm.name" placeholder="请输入链接类型名"/>
        </FormItem>
        <FormItem>
...
<script>
  ...
  import {deleteLinkType, listLinkTypePage, restoreLinkType, search} from "../../api/linkType";
    ...
    data() {
      return {
        columns: [
          ...
          {title: "链接类型名", key: "name", align: "left"},
          {
            title: "创建时间",
            key: "createTime",
            align: "left",
    ...
    methods: {
      changePage(current) {
        ...
        if (this.searchFlag) {
          ...
          this.listLinkTypePage(current, this.pageSize);
        }
      },
      edit(row) {
        this.$router.push({
      ...
      del(row) {
        if (row.status === 1) {
          this.$Modal.confirm({
            title: "会删除该类型所有链接，确认？",
            onOk: () => {
              deleteLinkType(row.id).then(res => {
      ...
      restore(row) {
        if (row.status === 1) {
          ...
          this.$Modal.confirm({
            title: "是否恢复",
            onOk() {
              restoreLinkType(row.id).then(res => {
                if (res.data.request === "success" && res.data.status === 200) {
      ...
      listLinkTypePage(page, count) {
        this.loading = true;
        listLinkTypePage(page, count).then(res => {
          this.tableData = res.data.data;
          this.loading = false;
          this.current = page;
          this.totalCount = parseInt(res.data.fields.totalCount);
